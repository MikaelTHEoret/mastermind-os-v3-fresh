...
try:
    ...
    catalogpath=ROOT/'memory-system/migrations/023_mastermind_native_catalog_v1.sql'
    cursor.execute(isolated(catalogpath.read_text()))
    receipt['sourceHashes'][catalogpath.relative_to(ROOT).as_posix()]=hashlib.sha256(catalogpath.read_bytes()).hexdigest()
    receipt['checks'].append('complete023 catalog extension compiles')
    ...
    CATALOG='mastermind.native.catalog'
    catalog_worker={'protocolVersion':2,'capabilities':[{'id':CORE,'version':1},{'id':NATIVE,'version':1},{'id':CATALOG,'version':1}]}
    catalog_input={'schemaVersion':1,'taskRef':native_input['taskRef'],'snapshotId':None,'cursor':None}
    catalog_job=uid()
    def enqueue_catalog(value=None,job=catalog_job,actor=ACTOR,node=NODE):
        value=catalog_input if value is None else value
        args=[job,sha(value),node,'fixture',actor,datetime.now(timezone.utc)+timedelta(minutes=30),json.dumps(value)]
        cursor.execute(f'SELECT * FROM {SCHEMA}.enqueue_mastermind_catalog_job_v1('+','.join(['%s']*7)+')',args)
        return cursor.fetchone()
    for change in [{'grantRef':'caller'},{'cursor':'a'*64},{'schemaVersion':True},{'taskRef':{'taskId':task,'project':123}}]:
        active_case='catalog invalid '+str(list(change))
        expected_error('22023',lambda change=change:enqueue_catalog({**catalog_input,**change}))
    for change in [{'taskRef':{'taskId':uid(),'project':'mastermind'}},{'taskRef':{'taskId':task,'project':'wrong'}}]:
        expected_error('42501',lambda change=change:enqueue_catalog({**catalog_input,**change}))
    expected_error('42501',lambda:enqueue_catalog(actor=FOREIGN))
    expected_error('42501',lambda:enqueue_catalog(node=OTHER))
    assert enqueue_catalog()[0]=='applied' and enqueue_catalog()[0]=='duplicate'
    assert enqueue_catalog({**catalog_input,'snapshotId':'b'*64})[0]=='conflict'
    assert enqueue_catalog(job=uid())[0]=='busy'
    assert enqueue_native({**native_input,'operationId':uid()})[0]=='busy'
    assert exchange(worker=native_worker)[4] is None
    leased_catalog=exchange(worker=catalog_worker)[4]
    assert leased_catalog['jobId']==catalog_job
    catalog_result={'kind':CATALOG,'ok':True,'schemaVersion':1,'taskRef':catalog_input['taskRef'],'snapshotId':'a'*64,
    ...
    cursor.execute(f"UPDATE {SCHEMA}.mastermind_context_tasks_v1 SET permission_scope=jsonb_set(permission_scope,'{{status}}','\"revoked\"') WHERE task_id=%s",(task,))
    expected_error('42501',lambda:submit(catalog_receipt,worker=catalog_worker))
    assert exchange(worker=catalog_worker)[4] is None
    cursor.execute(f'UPDATE {SCHEMA}.mastermind_context_tasks_v1 SET permission_scope=%s WHERE task_id=%s',(json.dumps(scope),task))
    assert catalog_receipt['receiptId'] in submit(catalog_receipt,worker=catalog_worker)[3]
    assert catalog_receipt['receiptId'] in submit(catalog_receipt,worker=catalog_worker)[3]
    ...
    cursor.execute(f"UPDATE {SCHEMA}.mastermind_context_tasks_v1 SET permission_scope=jsonb_set(permission_scope,'{{modules}}','[]') WHERE task_id=%s",(task,))
    cursor.execute(f'SELECT {SCHEMA}.mastermind_catalog_authorized_v1(%s,%s,%s,%s)',('fixture',ACTOR,json.dumps(catalog_input),json.dumps(catalog_result)))
