...
public static class MastermindFilesystemDirectorySession {
...
function Assert-NoAlternateStreams($Handle) {
    ...
    while ($true) {
        ...
        try {
            $streamOk = [MastermindFilesystemDirectorySession]::GetFileInformationByHandleEx(
...
function Get-DirectoryEvidence($Handle) {
    $tagSize = [Runtime.InteropServices.Marshal]::SizeOf(
        [type][MastermindFilesystemDirectorySession+FILE_ATTRIBUTE_TAG_INFO]
    )
    $tagBuffer = [Runtime.InteropServices.Marshal]::AllocHGlobal($tagSize)
    try {
        if (-not [MastermindFilesystemDirectorySession]::GetFileInformationByHandleEx(
            $Handle, 9, $tagBuffer, $tagSize
        )) { Stop-Unsafe }
        $tag = [Runtime.InteropServices.Marshal]::PtrToStructure(
            $tagBuffer, [type][MastermindFilesystemDirectorySession+FILE_ATTRIBUTE_TAG_INFO]
        )
    } finally {
        [Runtime.InteropServices.Marshal]::FreeHGlobal($tagBuffer)
    }
    $information = New-Object MastermindFilesystemDirectorySession+BY_HANDLE_FILE_INFORMATION
    if (-not [MastermindFilesystemDirectorySession]::GetFileInformationByHandle($Handle, [ref] $information)) {
        Stop-Unsafe
    }
...
function Assert-SameDirectory($Entry) {
    ...
    if ($current.attributes -ne $Entry.attributes -or $current.identity -ne $Entry.identity `
        -or $current.links -ne $Entry.links -or $current.tag -ne $Entry.tag) {
        Stop-Unsafe
    }
}
...
function Rename-HeldDirectory($Handle, [string] $Destination) {
    ...
    try {
        ...
        if (-not [MastermindFilesystemDirectorySession]::SetFileInformationByHandle(
    ...
    while ($true) {
        ...
        if ($request.command -eq 'acquire') {
            ...
            for ($index = 0; $index -lt $fullPaths.Count; $index += 1) {
                ...
                $handle = [MastermindFilesystemDirectorySession]::CreateFileW(
        ...
        if ($request.command -eq 'rename') {
            Rename-HeldDirectory $entry.handle $destination
            Assert-SameDirectory $entry
