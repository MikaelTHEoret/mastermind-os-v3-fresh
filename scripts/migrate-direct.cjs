...
// Explicit canonical memory configuration is required; no embedded fallback.
...
async function runMigration() {
  ...
  try {
    ...
    // Create RLS policies
    await sql`
      CREATE POLICY IF NOT EXISTS user_sources_config_policy ON user_sources_config
      FOR ALL USING (user_id = current_setting('app.current_user_id', true))
    `;
    
    await sql`
      CREATE POLICY IF NOT EXISTS user_file_cache_policy ON user_file_cache
      FOR ALL USING (user_id = current_setting('app.current_user_id', true))
    `;
