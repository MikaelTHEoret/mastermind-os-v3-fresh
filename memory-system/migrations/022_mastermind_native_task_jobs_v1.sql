BEGIN;
-- PREPARED ONLY. Add native reuse to the existing owner-scoped ledger, never a new queue.
CREATE OR REPLACE FUNCTION public.mastermind_native_input_valid_v1(p_input jsonb)
RETURNS boolean LANGUAGE plpgsql IMMUTABLE SET search_path=public,pg_temp AS $$
DECLARE field text;
BEGIN
 IF jsonb_typeof(p_input) IS DISTINCT FROM 'object' OR octet_length(p_input::text)>8192
   OR p_input->'schemaVersion' IS DISTINCT FROM '1'::jsonb OR p_input->>'action' IS DISTINCT FROM 'execute'
   OR p_input-ARRAY['schemaVersion','action','taskRef','specificationId','operationId','capability','candidateId','requirementsHash','inputSha256','arguments'] <> '{}'::jsonb
   ...
   OR jsonb_typeof(p_input->'capability') IS DISTINCT FROM 'string'
   ...
   OR (p_input->'taskRef' ? 'checkpointId' AND COALESCE(p_input->'taskRef'->>'checkpointId','') !~ '^[a-f0-9]{8}(-[a-f0-9]{4}){3}-[a-f0-9]{12}$')
   OR COALESCE(p_input->>'operationId','') !~ '^[a-f0-9]{8}(-[a-f0-9]{4}){3}-[a-f0-9]{12}$'
   OR COALESCE(p_input->>'capability','') !~ '^[a-z][a-z0-9_.-]{1,179}$'
   OR jsonb_typeof(p_input->'arguments') IS DISTINCT FROM 'object' THEN RETURN false; END IF;
 FOREACH field IN ARRAY ARRAY['specificationId','candidateId','requirementsHash','inputSha256'] LOOP
   IF jsonb_typeof(p_input->field) IS DISTINCT FROM 'string' OR COALESCE(p_input->>field,'') !~ '^[a-f0-9]{64}$' THEN RETURN false; END IF;
 END LOOP;
 IF EXISTS(SELECT 1 FROM jsonb_object_keys(p_input->'arguments') k WHERE left(k,length('_mastermind_'))='_mastermind_') THEN RETURN false; END IF;
 RETURN true;
END;
$$;
CREATE OR REPLACE FUNCTION public.mastermind_native_task_authorized_v1(p_household text,p_actor uuid,p_input jsonb)
RETURNS boolean LANGUAGE plpgsql STABLE SET search_path=public,pg_temp AS $$
BEGIN
 IF NOT public.mastermind_native_input_valid_v1(p_input) THEN RETURN false; END IF;
 ...
 AND t.permission_scope->'schemaVersion' IN ('1'::jsonb,'2'::jsonb)
 AND t.permission_scope->>'status'='active'
 ...
 WHERE m->'operations' @> '["module.call"]'::jsonb AND m->'capabilities' @> jsonb_build_array(p_input->>'capability')));
END;
$$;
...
BEGIN
 ...
 IF jsonb_array_length(p_worker -> 'capabilities') NOT BETWEEN 1 AND 3 THEN RETURN false; END IF;
 FOR item IN SELECT value FROM jsonb_array_elements(p_worker -> 'capabilities') LOOP
   IF jsonb_typeof(item) <> 'object' OR (item - 'id' - 'version') <> '{}'::jsonb
     ...
     OR item ->> 'id' NOT IN ('family-ecosystem.ensure-running','mastermind.core.status','mastermind.native.reuse')
...
BEGIN
 ...
 WHERE j.node_id=p_node_id AND j.capability='mastermind.native.reuse' AND j.state='queued' AND j.expires_at<=v_now;
 IF EXISTS(SELECT 1 FROM public.mastermind_node_jobs_v1 j WHERE j.node_id=p_node_id
 AND j.capability='mastermind.native.reuse' AND j.state IN ('queued','leased','running')) THEN
