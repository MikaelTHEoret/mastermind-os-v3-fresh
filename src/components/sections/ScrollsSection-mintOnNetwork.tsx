...
// Utility function for blockchain minting operations
...
interface MintingCallbacks {
  setMintingData: (updater: (prev: MintingData) => MintingData) => void;
  addTerminalLine: (message: string) => void;
}

export const mintOnNetwork = async (
  isL1: boolean,
  mintingData: MintingData,
  walletState: WalletState,
  callbacks: MintingCallbacks
) => {
  const { setMintingData, addTerminalLine } = callbacks;
