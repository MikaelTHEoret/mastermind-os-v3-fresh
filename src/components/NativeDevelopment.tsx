...
import NativeCapabilityInputs,{type NativeContract} from './NativeCapabilityInputs';
...
type Summary={specificationId:string;title:string;request:string;stage:string;candidateId:string|null;createdAt:string};
type TestCase={id:string;capability:string;input:Record<string,Json>;expected?:Json;expectedError?:{type:string;message:string}};
type Candidate={candidateId:string;status:string;identity:{expectedActiveRevision:string|null;manifest:{version:string};contracts:NativeContract[]};
  testReceipts:{outcome:string}[];testRuns?:{status:string;completedCases?:number;caseCount?:number;error?:string}[]};
type Spec={specificationId:string;holderId:string;candidateId:string|null;activeRevision:string|null;
  identity:{request:string;title?:string;stage:string;decision:string;moduleId:string|null;missing:string[];expectedActiveRevision?:string|null;
    requirements:null|{requirements:string[];compatibility?:string;contracts:NativeContract[];tests:{cases:TestCase[]};sourceReferences?:unknown[]};
    sourceRef:null|{commit:string;path:string};catalogMatches:{name:string;available:boolean;basis:string}[]};
  candidate?:Candidate;revisions?:{candidateId:string;acceptedAt:string}[];
...
async function api(action:string,body?:Record<string,unknown>,signal?:AbortSignal) {
  const query=new URLSearchParams({action,...(body && typeof body.specificationId==='string' && action==='specification'?{specificationId:body.specificationId}:{})});
  const isRead=body===undefined || action==='specification' && Object.keys(body).length===1;
  const response=await fetch(isRead?`/api/modules?${query}`:'/api/modules',isRead?{cache:'no-store',signal}:{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action,...body}),signal});
  ...
  if(!response.ok || result.ok===false)throw new Error(typeof result.error==='string'?result.error:'The saved result could not be verified. Refresh its status before continuing.');
  return result;
}
...
export default function NativeDevelopment() {
  const [catalog,setCatalog]=useState<Catalog|null>(null),[history,setHistory]=useState<Summary[]>([]),[selected,setSelected]=useState<Spec|null>(null);
  ...
  const refresh=useCallback(async(signal?:AbortSignal)=>{
    ...
    try {
      ...
      if(!Array.isArray(availability.recipes)||!Array.isArray(saved.specifications))throw new Error('The native development catalog returned an incomplete response.');
      ...
      setCatalog(availability);setHistory(saved.specifications);setSelected(detail?.specification ?? null);
  ...
  useEffect(()=>{
    alive.current=true;const controller=new AbortController();void refresh(controller.signal);
  ...
  const choose=async(id:string)=>{
    ...
    selection.current=id;setPreflight(null);setExampleResult(undefined);setRollback('');setLoading(true);
    try {const detail=await api('specification',{specificationId:id});if(alive.current && selection.current===id)setSelected(detail.specification);}
    catch(problem){setError(problem instanceof Error?problem.message:'Saved work is unavailable.');}
  ...
  const act=async(action:string,payload:Record<string,unknown>,message:string)=>{
    ...
    try {
      ...
      if(result.specification){selection.current=result.specification.specificationId;setSelected(result.specification);setExampleResult(undefined);setRollback('');}
      if(result.preflight)setPreflight({...result.preflight,candidateId:String(payload.candidateId)});
  ...
  const reusing=selected?.identity.decision==='reuse';
  const active=!!selected?.activeRevision && ((!!candidate && selected.activeRevision===candidate.candidateId) || reusing && selected.identity.expectedActiveRevision===selected.activeRevision);
  const recipes=catalog?.recipes ?? [];
  const canAct=!!catalog?.available && !busy && !loading;
  const base=selected && candidate?{id:selected.holderId,candidateId:candidate.candidateId}:null;
  const revisionOptions=(selected?.revisions ?? []).filter(item=>item.candidateId!==selected?.activeRevision);
  const successfulExamples=selected?.identity.requirements?.tests.cases.filter(item=>!item.expectedError).slice(0,4) ?? [];
  const inputContracts=candidate?.identity.contracts ?? selected?.identity.requirements?.contracts ?? [];
  ...
  useEffect(()=>{setPreflight(null);},[selected?.specificationId,selected?.candidateId,selected?.activeRevision,testStatus]);
  ...
  return <section className="native-development" aria-label="Native development" style={panel}>
    ...
    {!!history.length && <label style={{display:'block',marginTop:18}}>Resume saved work
      <select aria-label="Saved development request" style={{...field,marginTop:5}} value={selected?.specificationId ?? ''} disabled={busy} onChange={event=>void choose(event.target.value)}>
    ...
    {selected && <article style={{marginTop:18,borderTop:'1px solid #284956',paddingTop:14}}>
      ...
      <div style={{display:'flex',flexWrap:'wrap',gap:8,marginTop:14}}>
        {!candidate && selected.identity.stage==='specified' && <button style={button} disabled={!canAct} onClick={()=>void act('specification_stage',{specificationId:selected.specificationId,operationId:crypto.randomUUID()},'The committed source has been staged. It is ready for isolated behavioral tests.')}>Stage reviewed source</button>}
        ...
        {base && !active && <button style={button} disabled={!canAct || running || !currentPreflight?.ready || candidate?.testReceipts.at(-1)?.outcome!=='passed'} onClick={()=>void act('candidate_promote',{...base,expectedActiveRevision:selected.activeRevision,operationId:crypto.randomUUID()},selected.activeRevision?'The tested revision is active. Its earlier accepted revision remains in history.':'The tested revision is active.')}>Activate tested version</button>}
      </div>
      ...
      {active && inputContracts.length>0 && <NativeCapabilityInputs key={selected.specificationId+selected.activeRevision} contracts={inputContracts} disabled={!canAct} onRun={runInputs}/>}
      ...
      </label><button style={{...button,marginTop:8}} disabled={!canAct || !rollback || running} onClick={()=>void act('candidate_rollback',{id:selected.holderId,candidateId:rollback,expectedActiveRevision:selected.activeRevision,operationId:crypto.randomUUID()},'The selected accepted revision has been restored.')}>Restore selected version</button></div>}
