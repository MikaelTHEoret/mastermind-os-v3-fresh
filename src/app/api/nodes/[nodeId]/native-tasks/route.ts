...
import {enqueueOwnerNativeTaskJob} from '@/lib/node-exchange/store';
...
export async function POST(request: Request, context: RouteContext): Promise<Response> {
  ...
  try {
    authorizeOwnerRequest(request, `/api/nodes/${nodeId}/native-tasks`, true);
    ...
    const result = await enqueueOwnerNativeTaskJob(getMemoryDb(),nodeId,input);
