...
- On 2026-08-22, the authenticated supervisor safely drained the running server, replaced the Minecraft control agent with generation 2, and restarted the Family Server as managed Java PID 29064. Family Core 0.5.0 loaded with chat capture enabled and reauthenticated at 09:40:09. The new intake path is active; a fresh addressed parent-chat receipt is still required before changing conversation routing from `implemented` to `live-verified`.
