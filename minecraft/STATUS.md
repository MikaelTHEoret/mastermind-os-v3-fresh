...
- A source-only `stay_alive` emergency selector now consumes the already validated v1 health/hunger snapshot and deterministically selects stable, escape, respawn, or food-needed outcomes. It honors the kill switch, stale-state gate, existing physical-action lock, and a dispatch cooldown; low hunger remains a visible unavailable result until the typed eat action exists. Six focused tests pass. Survival remains `stubbed` and default-off because threat telemetry, eating, and emergency preemption are not complete.
