...
- Hybrid body foundation: the disabled-by-default `family-core` 0.3.1 authenticated server bridge is managed and live-verified. A fresh file-only token is created after exact artifact verification, rechecked immediately before spawn, bound to the exact hello identity and disabled feature state, retained only for an authenticated surviving server, and removed on normal exit. Rotation, tamper, orphan, recovery, launch-order, cleanup, sanitized-status, isolated loopback socket, disposable real Fabric-server boot, live Family Server authentication, and empty-server heartbeat continuity checks pass.
- The schema-v2 authenticated first-party artifact registry, verified-snapshot gate, atomic install/rollback boundary, exact launch binding, and bounded status endpoint are active for `family-core`. Generation `5ee88d6abd2d7f4067d51c8f81c6b81dfd6509f5949471e5d323516dac6d5aaa` is bound to verified backup `bkp-d9bc22fe17cf8486ed361a57bb3faad0`; Fabric live-loaded the pinned 48,754-byte 0.3.1 JAR, authenticated the loopback bridge, and kept Computer plus the other seven runtime feature flags disabled. Rollback to the accepted 0.3.0 artifact is available. Third-party mod mutations remain deliberately blocked until combined dependency-graph validation is added.
...
- Separate server-only Fabric `family-core` for Minecraft 26.2 / Java 25. The pinned 0.3.1 artifact is installed through the managed first-party lane and live-verified in the Family Server. Its authenticated server bridge and deterministic `/computer help` and `/computer status` surface are enabled; every other Computer request receives a typed rejection without a model call. The live launch authenticated with exactly the `computer.request` capability, remained continuously connected past the empty-server pause boundary, and `MISS_LENKA` completed the player-visible help/status check on 2026-08-22. Chat capture, identity events, administration, shutdown, companion telemetry, and companion events remain disabled.
