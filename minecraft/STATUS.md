...
- Hybrid body foundation: the disabled-by-default `family-core` 0.4.0 authenticated server bridge is managed and live-verified. A fresh file-only token is created after exact artifact verification, rechecked immediately before spawn, bound to the exact hello identity and enabled capability set, retained only for an authenticated surviving server, and removed on normal exit. Rotation, tamper, orphan, recovery, launch-order, cleanup, sanitized-status, isolated loopback socket, disposable real Fabric-server boot, live Family Server authentication, and empty-server heartbeat continuity checks pass.
- The schema-v2 authenticated first-party artifact registry, verified-snapshot gate, atomic install/rollback boundary, exact launch binding, and bounded status endpoint are active for `family-core`. Generation `b21e99cbd61183dc71631f7833979c477c3c7f4522f71833871217d2d509b96a` is bound to verified backup `bkp-11b63348072b30a539b347740236fb7f`; Fabric live-loaded the pinned 50,003-byte 0.4.0 JAR with SHA-256 `1a9babbce78c4105a71a9bb35c121cad4d567e988d2035f2cdbc1667324105f1`, authenticated the loopback bridge, and advertised only deterministic Computer requests plus identity events. Rollback to the accepted 0.3.1 artifact is available. Third-party mod mutations remain deliberately blocked until combined dependency-graph validation is added.
...
- A reproducible 50,772-byte `family-core` 0.5.0 candidate with SHA-256 `9c3138dc8c7830b514a9714d4f1df329a220fa87ec06f53d6bc516a03b333ac8` implements a separately authenticated `chat.capture` capability. It forwards bounded signed public text, resolves identity centrally, and emits an internal event only. It makes no model call, writes no profile, and sends no companion response. The candidate is built and tested but not yet promoted to the live server.
...
- Separate server-only Fabric `family-core` for Minecraft 26.2 / Java 25. The pinned 0.4.0 artifact is installed through the managed first-party lane and live-verified in the Family Server. Its authenticated server bridge and deterministic `/computer help` and `/computer status` surface are enabled; every other Computer request receives a typed rejection without a model call. The live launch authenticated with exactly the `computer.request` and `identity.events` capabilities, remained continuously connected past the empty-server pause boundary, and `MISS_LENKA` completed the player-visible help/status check on 2026-08-22. The same bound parent UUID then joined, resolved as the sole parent identity, and remained present in the authenticated session. Chat capture, administration, shutdown, companion telemetry, and companion events remain disabled.
