...
- Deterministic dual-character intake now classifies explicit Computer requests, exact companion addressing, direct replies, and a bounded continuation window. Authenticated chat receipts feed a redacted counter/status projection that stores no chat text, name, or player identifier. Model calls, persona speech, profile capture, and task execution remain disabled.
- On 2026-08-22, the authenticated supervisor safely drained the running server, replaced the Minecraft control agent with generation 2, and restarted the Family Server as managed Java PID 29064. Family Core 0.5.0 loaded with chat capture enabled and reauthenticated at 09:40:09. Bound parent `MISS_LENKA` sent `hello alchemist` at 09:42:47; the same Java-to-agent connection created at 09:40:09 remained established afterward with no protocol rejection or reconnect. Deterministic companion conversation routing is therefore live-verified. Conversation generation, profile capture, model reasoning, and persona speech remain disabled.
- After a power outage interrupted the dashboard rollout, the signed stale supervisor state and free controlled ports were reconciled before restart. The production dashboard now preserves the prior service-control surface while overlaying the corrected Minecraft route (SHA-256 `DE450A4D9ABC16F47466F63AF3D1EEEFD499A3CEB29A14B7613B92AC3D3F8DEE`) on build `C_mckwiS4CRjkvjj8VqUf`. Live checks returned 200 for service inventory, the 12-feature brain status, and redacted conversation status; all brain flags remain off and `storesChatContent` is false. The Family Server restarted as managed Java PID 22480, Geyser reached ready, and Family Core reauthenticated at 10:32:50. Both the untouched prior dashboard build and the clean corrected build remain available as local rollback material.
- Later on 2026-08-22, the bounded provider-backed companion conversation lane was enabled and live-verified through `The_AlChemist___`: a parent message received an in-world model reply, and a follow-up correction prevented the companion from responding to its own account. The persona prompt now favors concise, natural Minecraft-friend speech and keeps safety policy implicit unless it is relevant. This verifies conversation and its constrained model lane, not profiles or autonomous gameplay.
- The first deterministic natural-language physical-task slice is implemented in source and remains default-off pending controlled live acceptance. Explicit parent/child requests can compile to typed follow, navigate, bounded explore, whitelisted gather, and cancel actions; guest requests fail before dispatch, ambiguous wording falls back to conversation, one authoritative physical action remains the limit, and recognized tasks make zero model calls even when conversation/model access is unavailable. Focused sequential acceptance is 51/51 passing. It is not installed or advertised as live-verified.
- The v1 `stay_alive` emergency controller consumes already validated health/hunger snapshots and deterministically selects stable, escape, respawn, or food-needed outcomes. It honors the kill switch and stale-state gate, preempts ordinary physical work before an emergency action, serializes overlapping ticks, and applies a dispatch cooldown; low hunger remains a visible unavailable result until the typed eat action exists. The implemented v1 scope remains default-off until live activation, while richer threat telemetry and eating still require v2.
...
- Only the bounded `family-core` bridge and deterministic Computer/identity/public-chat transport features are installed into the server artifact lock. The separately gated companion conversation/model lane is now live-verified through the existing client bridge; profiles, persistent memory, survival, and physical-task execution remain off. A one-use typed administration plan temporarily granted `MISS_LENKA` operator access to execute `/setworldspawn`; the server set the Overworld spawn to `149, 63, -645`, the paired de-op plan succeeded, and `ops.json` was verified empty afterward.
...
- `family-agent-bridge-0.1.0.jar` — 91,600 bytes — SHA-256 `78BF942996A91CC621294D3FE3D081731EE0FC2039E43668567F87467048F69B`
